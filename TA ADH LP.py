...
# In[13]:
dataset = [[process[x]]+extraction[x]+[labelfix[x]] for x in range(len(extraction))]
...
def split(dtrain,dtest):
    train = [dtrain[x][0:-1] for x in range(len(dtrain))]
    test = [dtest[x][0:-1] for x in range(len(dtest))]
    labeltrain= [dtrain[x][-1]for x in range(len(dtrain))]
    labeltest= [dtest[x][-1]for x in range(len(dtest))]
    return train,test,labeltrain,labeltest
index=[]
performance=[]
for x in range(5):
    trainf,testf,labeltrain,labeltest=split(train[x],test[x])
    model = RandomForestClassifier(rf_trees=1,rf_samples=1000)
